...
for ($i = 0; $i < $contador; $i++) {
    if (!empty($_POST["IdBasedatos"][$i]) || !empty($_POST["TiposDeServicio"][$i]) || !empty($_POST["NumeroDeTickeT"])) {
        $ProContador++;
        $placeholders[] = "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?)";
        $values[] = $_POST["IdBasedatos"][$i];
        $values[] = $_POST["TiposDeServicio"][$i];
        $values[] = $_POST["TurnoEnTurno"][$i];
        $values[] = $_POST["NumeroDeTickeT"];
        ...
        $values[] = $_POST["TotalVentas"][$i];
        $values[] = $_POST["DescuentoAplicado"][$i];
        $values[] = $_POST["FormaDePago"][$i];
        $values[] = $_POST["PagoReal"][$i];
        $values[] = $_POST["Cambio"][$i];
