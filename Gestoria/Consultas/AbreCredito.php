...
$resultset = mysqli_query($conn, $sql) or die(json_encode(array("statusCode" => 500, "error" => mysqli_error($conn))));

if ($resultset) {
    $row = mysqli_fetch_assoc($resultset);

    if ($row && $row['Fk_tipo_Credi'] == $Fk_tipo_Credi && $row['ID_H_O_D'] == $ID_H_O_D && $row['Nombre_Cred'] == $Nombre_Cred && $row['Fecha_Apertura'] == $Fecha_Apertura && $row['Fecha_Termino'] == $Fecha_Termino && $row['Fk_Sucursal'] == $Fk_Sucursal) {
