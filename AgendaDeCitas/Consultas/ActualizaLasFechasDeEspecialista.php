<?php
if (!empty($_POST['Fechasporactualizar']) && !empty($_POST['Id_Fecha'])) {
    include_once 'db_connection.php';

    $fechasPorActualizar = $_POST['Fechasporactualizar'];
    $idFechas = $_POST['Id_Fecha'];
    $success = true;

    foreach ($idFechas as $key => $val) {
        $valfecha = $fechasPorActualizar[$key];

        $sql = "UPDATE `Fechas_EspecialistasExt` 
                SET `Fecha_Disponibilidad`='$valfecha'
