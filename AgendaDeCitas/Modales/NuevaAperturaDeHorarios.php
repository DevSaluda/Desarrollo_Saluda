...
SucursalesCorre.ID_SucursalC, SucursalesCorre.Nombre_Sucursal, Personal_Medico_Express.Medico_ID, Personal_Medico_Express.Nombre_Apellidos 
...
AND Programacion_MedicosExt.ID_H_O_D ='" . $row['ID_H_O_D'] . "' 
            ...
            <div class="col">
                ...
                <div class="input-group mb-3">
                    ...
                    <select id="FechasSeleccionadasxd" class="form-control" name="FechaAsignadaParaHoras[]" multiple>
                        <?php
                        $query = $conn->query("SELECT ID_Fecha_Esp, Fecha_Disponibilidad, Fk_Programacion FROM Fechas_EspecialistasExt 
                            WHERE ID_Fecha_Esp NOT IN (SELECT FK_Fecha FROM Horarios_Citas_Ext WHERE FK_Fecha IS NOT NULL) 
            ...
            <div class="col">
                ...
                <div class="input-group mb-3">
                    ...
                    <select id="SeleccionHorarios" class="form-control" name="HorasAGuardar[]" multiple>
                        <?php
                        $queryHoras = $conn->query("SELECT Horario_Disponibilidad FROM Horarios_Citas_Ext 
                            WHERE FK_Fecha = {$_POST['id']} AND ID_H_O_D = '{$row['ID_H_O_D']}'");
                        $horasAsignadas = [];
                        while ($horas = $queryHoras->fetch_assoc()) {
                            $horasAsignadas[] = $horas['Horario_Disponibilidad'];
                        }
                        while ($date_incr < $final) {
                            $date_incr = $date_obj->format('H:i:s');
                            if (!in_array($date_incr, $horasAsignadas)) {
