<?php
// Desarrollo_Saluda/AdminPOS/Modales/EdicionFormasPagoTicket_Debug.php
// Versión de debug para identificar el error 500

error_reporting(E_ALL);
ini_set('display_errors', 1);

echo "<!-- Debug: Iniciando modal -->\n";
...
try {
    ...
    echo "<!-- Debug: folioTicket = $folioTicket, folioSucursal = $folioSucursal -->\n";
    ...
    echo "<!-- Debug: Verificando conexión -->\n";
    if (!$conn) {
    ...
    echo "<!-- Debug: Obteniendo datos del ticket -->\n";
    $ticket = $result->fetch_assoc();
    
    echo "<!-- Debug: Ticket obtenido -->\n";
    ...
    echo "<!-- Debug: Parseando formas de pago -->\n";
    $formasPagoActuales = parsearFormasPago($ticket['FormaDePago'], $ticket['CantidadPago']);
    
    echo "<!-- Debug: Generando HTML del modal -->\n";
    
} catch (Exception $e) {
    echo '<div class="alert alert-danger">Error: ' . $e->getMessage() . '</div>';
    echo '<div class="alert alert-info">Archivo: ' . $e->getFile() . '</div>';
    echo '<div class="alert alert-info">Línea: ' . $e->getLine() . '</div>';
    exit;
}
?>
...
<!-- Debug: Modal generado exitosamente -->
