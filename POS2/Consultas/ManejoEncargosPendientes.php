...
function abonarEncargo($conn, $identificadorEncargo, $montoAbonado) {
    ...
    $query = "SELECT IdentificadorEncargo FROM Encargos_POS WHERE IdentificadorEncargo='$identificadorEncargo'";
    $result = mysqli_query($conn, $query);

    if (mysqli_num_rows($result) > 0) {
        $primero = true;
        while ($row = mysqli_fetch_assoc($result)) {
            $identificador = $row['IdentificadorEncargo'];

            // Asignar el monto abonado solo al primer producto
            if ($primero) {
                $query = "UPDATE Encargos_POS SET MontoAbonado = MontoAbonado + '$montoAbonado' WHERE IdentificadorEncargo='$identificador'";
                $primero = false;
            } else {
                // No actualizar el monto abonado para los productos subsiguientes
                $query = "UPDATE Encargos_POS SET MontoAbonado = MontoAbonado WHERE IdentificadorEncargo='$identificador'";
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['idEncargo']) && isset($_POST['accion']) && isset($_POST['productosSeleccionados'])) {
        ...
        if ($accion === 'entregar' || $accion === 'saldar') {
            ...
            foreach ($productosSeleccionados as $identificadorProducto) {
                $query = "UPDATE Encargos_POS SET Estado='$nuevoEstado' WHERE IdentificadorEncargo='$identificadorProducto'";
                if (!mysqli_query($conn, $query)) {
        ...
        if ($accion === 'cancelar' && isset($_POST['motivoCancelacion'])) {
            ...
            foreach ($productosSeleccionados as $identificadorProducto) {
                ...
                $query = "SELECT * FROM Encargos_POS WHERE IdentificadorEncargo = '$identificadorProducto'";
                $result = mysqli_query($conn, $query);
                $producto = mysqli_fetch_assoc($result);

                // Insertar en la tabla EncargosCancelados
                $queryCancelacion = "INSERT INTO EncargosCancelados 
                    (IdentificadorEncargo, Cod_Barra, Nombre_Prod, EstadoAnterior, EstadoNuevo, MotivoCancelacion, FechaCancelacion, Fk_sucursal, Cantidad, Precio_Venta, ID_H_O_D)
                    VALUES ('{$producto['IdentificadorEncargo']}', '{$producto['Cod_Barra']}', '{$producto['Nombre_Prod']}', '{$producto['Estado']}', 'Cancelado', '$motivo', NOW(), '{$producto['Fk_sucursal']}', '{$producto['Cantidad']}', '{$producto['Precio_Venta']}', '{$producto['ID_H_O_D']}')";
                mysqli_query($conn, $queryCancelacion);

                // Actualizar el estado del producto en Encargos_POS
                $queryActualizar = "UPDATE Encargos_POS SET Estado = 'Cancelado' WHERE IdentificadorEncargo = '$identificadorProducto'";
